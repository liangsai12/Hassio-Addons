...
def on_wake( va_config ):
  """唤醒后的处理函数"""
  ha_api.play_audio_file(va_config["media_player"], "ding.wav", va_config["ha_base_url"] )

def on_command( va_config ):
  """读入语音命定后的处理函数"""
  ha_api.play_audio_file(va_config["media_player"], "dong.wav", va_config["ha_base_url"] )
